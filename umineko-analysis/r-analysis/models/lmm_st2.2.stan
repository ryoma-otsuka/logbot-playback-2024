...
model {
  ...
  sigma_u ~ normal(0, 10); // Weakly informative prior | Half-Normal Distribution
  sigma_Y ~ normal(0, 10); // Weakly informative prior | Half-Normal Distribution
  
  // Likelihood
  z ~ normal(0, 1);
  Y ~ student_t(2.2, mu, sigma_Y);
}
...
generated quantities {
  ...
  for (i in 1:N)
    Y_pred[i] = student_t_rng(2.2, mu[i], sigma_Y);
    
  for (i in 1:N_new) {
    vector[K] u_new;
    u_new[GID_new[i]] = sigma_u * normal_rng(0, 1); 
    Y_pred_new[i] = student_t_rng(2.2, X_new[i] * beta + alpha + u_new[GID_new[i]], sigma_Y);
  }
  
  for (i in 1:N)
    log_lik[i] = student_t_lpdf(Y[i] | 2.2, mu[i], sigma_Y);
}
